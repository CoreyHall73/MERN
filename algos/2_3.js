...
function symmetricDifferences(numsA, numsB) {
    ...
    if (numsA.length === 0 || numsB.length === 0) {
        return returnArray.concat(numsA, numsB);
    }

    while (i < numsA.length && j < numsB.length) {

        if (!numsB.includes(numsA[i]) && !returnArray.includes(numsA[i])) {
        ...
        if (!numsA.includes(numsB[j]) && !returnArray.includes(numsB[j])) {
            returnArray.push(numsB[j]);
            j++
        } else {
            j++;
        }
